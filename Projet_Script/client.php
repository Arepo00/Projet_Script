...
if (mysqli_query($conn,$sql)) {
    ...
    self::$successMsg = "New Account created successfully";
...
public static function  selectAllClients2($tableName,$conn){

    //select all the client from database, and inset the rows results in an array $data[]
    
    
    $sql="SELECT id_cl, firstname, lastname, email, password FROM $tableName"; $result = mysqli_query($conn, $sql);
...
$sql="SELECT id_cl, firstname, lastname, email FROM $tableName"; $result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0) {
...
/*static function selectClientByem($tableName,$conn,$em){

    //select a client by id, and return the row result
    
    $sql="SELECT id_cl FROM $tableName WHERE email='$em'"; 
...
$row =mysqli_fetch_assoc($result);
    ...
    $sql="SELECT firstname, lastname, email FROM $tableName WHERE id_cl='$id'"; 
    $result=mysqli_query($conn, $sql);
...
$sql = "UPDATE $tableName SET lastname='$client->lastname', firstname='$client->firstname', email='$client->email' WHERE id_cl='$id'";
if (mysqli_query($conn,$sql)) {
...
header("Location:Signup.php");
    ...
    $sql= "DELETE FROM $tableName WHERE id_cl='$id'";
